#!/usr/bin/env python3
"""
FIXED WEBSITE LINK GENERATOR - Unique ID + Smooth Scroll
"""
import os
import hashlib
import time
...
def update_amazing_directory_with_enhanced_products():
    """Update the amazing directory with enhanced product pages"""
    try:
        # Load current products
        with open('data/products.json', 'r') as f:
            products = json.load(f)
        
        # Create enhanced articles directory if it doesn't exist
        enhanced_dir = Path('amazing/articles-enhanced')
        enhanced_dir.mkdir(exist_ok=True)
        
        log_message = "Creating enhanced product pages...\n"
        
        for product in products[:3]:  # First 3 products for demo
            # Generate enhanced HTML
            enhanced_html = create_enhanced_article_html(product)
            
            # Save enhanced page
            filename = f"item-{product['asin']}.html"
